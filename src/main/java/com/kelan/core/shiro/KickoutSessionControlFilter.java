...
public class KickoutSessionControlFilter extends AccessControlFilter {
	...
	public void setCacheManager(CacheManager cacheManager) {
		this.cache = cacheManager.getCache("sampleCache1");
	}
	...
	protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue)
		...
		if (!subject.isAuthenticated() && !subject.isRemembered()) {
			...
			if ("/login".equals(httpRequest.getRequestURI())) {
				httpResponse.setHeader("Set-Cookie", "JSESSIONID=deleteMe; Path=/; HttpOnly");
			}
			return true;
		}
		HttpServletRequest httpRequest = (HttpServletRequest) request;
		log.info("登录拦截检查" + " " + httpRequest.getRequestURI());
		...
		if (session.getAttribute("kickout") != null) {
			...
			try {
				subject.logout();
				log.info("用户"+username+"重复在线->kickoutfirst");
			} catch (Exception e) {
				log.error(e);
			}
	...
	protected void postHandle(ServletRequest request, ServletResponse response) throws Exception {
		...
		if (!subject.isAuthenticated() && !subject.isRemembered()) {
			if (httpResponse.containsHeader("Set-Cookie")) {
				//log.info(httpRequest.getRequestURI()+httpResponse.getHeader("Set-Cookie"));
				httpRequest.getSession().invalidate();
			}
		} else if (subject.isAuthenticated() || subject.isRemembered()) {
			//log.info(httpRequest.getRequestURI()+httpResponse.getHeader("Set-Cookie"));
			if ("/login".equals(httpRequest.getRequestURI())) {
				isAccessAllowed(request, response, null);
			}
		}
	}
}
